...
function handleUpvote(btn, postId) {
    ...
    .then(function(data) {
        ...
        if (data.status === 'added') { btn.classList.add('active'); } else { btn.classList.remove('active'); }
    })
...
function handleFollow(btn, userId) {
    ...
    .then(function(data) {
        ...
        if (data.status === 'followed') { btn.textContent = 'Following'; btn.classList.add('following'); } else { btn.textContent = 'Follow'; btn.classList.remove('following'); }
    })
...
function showToast(message) {
    ...
    toast.innerHTML = '<span class="flash-icon">✓</span> ' + message;
