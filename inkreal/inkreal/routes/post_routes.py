...
from utils.helpers import generate_slug, truncate_text, get_cover_image
...
def write():
    ...
    if request.method == "POST":
        ...
        if len(content) < 50:
            flash("Write at least 50 characters.", "error")
            return render_template("write.html")
        db = get_db()
        ...
        post_data = {
            ...
            "upvotes": 0, "upvoted_by": [], "bookmarks": 0, "bookmarked_by": [],
            "comment_count": 0, "view_count": 0,
            "human_score": human_analysis["score"],
            "human_label": human_analysis["label"],
            "word_count": len(content.split()), "featured": False
        }
        db.collection("posts").document(post_id).set(post_data)
        db.collection("users").document(session["user_id"]).update({"post_count": (user.get("post_count", 0) + 1)})
        posts_ref = db.collection("posts").where("author_id", "==", session["user_id"]).where("status", "==", "published").order_by("created_at", direction="DESCENDING")
        ...
        db.collection("users").document(session["user_id"]).update({
            ...
            "streak_longest": max(streak_data["longest"], user.get("streak_longest", 0))
        })
...
def view(post_id):
    ...
    more_posts_ref = db.collection("posts").where("author_id", "==", post.get("author_id")).where("status", "==", "published").order_by("created_at", direction="DESCENDING").limit(4)
    more_posts = []
    ...
    return render_template("post_detail.html", post=post, comments=comments, is_upvoted=is_upvoted, is_bookmarked=is_bookmarked, more_posts=more_posts[:3])
...
def delete(post_id):
    ...
    if user_doc.exists:
        user = user_doc.to_dict()
        db.collection("users").document(session["user_id"]).update({"post_count": max(0, user.get("post_count", 1) - 1)})
