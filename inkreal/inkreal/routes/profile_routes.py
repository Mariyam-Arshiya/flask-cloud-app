...
def view(handle):
    ...
    return render_template("profile.html", user=user, posts=posts, streak=streak_data, streak_level=streak_level, is_following=is_following, is_own_profile=is_own_profile)
...
def settings():
    ...
    if request.method == "POST":
        ...
        update_data = {"name": name, "bio": bio, "website": website, "location": location, "avatar_letter": name[0].upper()}
        if avatar_url:
