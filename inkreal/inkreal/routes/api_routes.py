...
def upvote(post_id):
    ...
    if session["user_id"] in upvoted_by:
        ...
        new_count = max(0, post.get("upvotes", 1) - 1)
        post_ref.update({"upvotes": new_count, "upvoted_by": upvoted_by})
        return jsonify({"status": "removed", "count": new_count})
    else:
        ...
        if author_id:
            ...
            if author_doc.exists:
                ...
                db.collection("users").document(author_id).update({"total_upvotes_received": author.get("total_upvotes_received", 0) + 1})
...
def bookmark(post_id):
    ...
    if session["user_id"] in bookmarked_by:
        ...
        post_ref.update({"bookmarks": max(0, post.get("bookmarks", 1) - 1), "bookmarked_by": bookmarked_by})
        return jsonify({"status": "removed"})
    else:
        bookmarked_by.append(session["user_id"])
        post_ref.update({"bookmarks": post.get("bookmarks", 0) + 1, "bookmarked_by": bookmarked_by})
...
def comment(post_id):
    ...
    comment_data = {
        ...
        "created_at": now, "upvotes": 0
    }
    db.collection("posts").document(post_id).collection("comments").document(comment_id).set(comment_data)
    post_doc = db.collection("posts").document(post_id).get()
    if post_doc.exists:
        post = post_doc.to_dict()
        db.collection("posts").document(post_id).update({"comment_count": post.get("comment_count", 0) + 1})
...
def follow(user_id):
    ...
    if session["user_id"] in followers:
        ...
        target_ref.update({"followers": followers, "followers_count": len(followers)})
        current_ref.update({"following": following, "following_count": len(following)})
